...
function ActivateBlips()
    ...
    AddTextComponentString("Apreensão de Veículos")
...
AddEventHandler('HRP:Impound:VehicleUnimpounded', function (data, index)
	...
	_ESX.ShowNotification("Seu veículo com a placa: " .. data.plate .. " foi desapreendido!")
...
AddEventHandler('HRP:Impound:CannotUnimpound', function ()
	_ESX.ShowNotification("Seu veículo não pode ser desapreendido neste momento, você tem bastante dinheiro?");
end)
...
function ShowImpoundMenu (action)
	...
	if (IsPedInAnyVehicle(GetPlayerPed(PlayerId()))) then
		_ESX.ShowNotification("Saia do veículo primeiro")
		return
	end
	...
	if (vehicle ~= nil) then
		...
		if(_VehicleAndOwner == nil) then
			_ESX.ShowNotification('Dono do veículo desconhecido, não é possivel realizar esta ação');
			return
		end
		...
		_ESX.ShowNotification('Nenhum veículo nas proximidades');
	end
...
RegisterNUICallback('impound', function(data, cb)
	...
	if (veh.plate:gsub("%s+", "") ~= data.plate:gsub("%s+", "")) then
		_ESX.ShowNotification("O veículo solicitado e o veículo mais próximo não correspondem");
		return
	end
	...
	_ESX.Game.DeleteVehicle(v);
	
	ClearPedTasks(PlayerPedId())
	
	ImpoundVehicle(_ESX.Game.GetClosestVehicle())

	DisableImpoundMenu()
end)

RegisterNUICallback('unimpound', function(plate, cb)
	Citizen.Trace("Desapreendendo:" .. plate)
		...
		if(_DependenciesLoaded) then
			...
			if (GetDistanceBetweenCoords(_Impound.RetrieveLocation.X, _Impound.RetrieveLocation.Y, _Impound.RetrieveLocation.Z,
				...
				if (_CurrentAction ~= "retrieve") then
					...
					_ESX.ShowHelpNotification("Pressione ~INPUT_CONTEXT~ para retirar um veículo apreendido");
			...
			elseif (GetDistanceBetweenCoords(_Impound.StoreLocation.X, _Impound.StoreLocation.Y, _Impound.StoreLocation.Z,
				...
				if (_CurrentAction ~= "store" and (_XPlayer.job.name == "police")) then

					_CurrentAction = "store"
					_ESX.ShowHelpNotification("Pressione ~INPUT_CONTEXT~ para apreender veículo");
				...
				for i, location in ipairs(_Impound.AdminTerminalLocations) do
					...
					PlayerPedCoords.x, PlayerPedCoords.y, PlayerPedCoords.z, false) < 3) then
						...
						if (_CurrentAction ~= "admin" and (_XPlayer.job.name == "police" or _XPlayer.job.name == "mecano")) then
							...
							_ESX.ShowHelpNotification("Pressione ~INPUT_CONTEXT~ para abrir o painel policial");
						end
...
RegisterNetEvent("esx_apreensao:openApreensaoMenu")
AddEventHandler("esx_apreensao:openApreensaoMenu", function()
...
function ImpoundVehicle(vehicle)
	...
	_ESX.Game.DeleteVehicle(vehicle) 
end
