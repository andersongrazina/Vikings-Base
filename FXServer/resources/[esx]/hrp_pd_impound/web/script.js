$(document).ready(function () {
	...
	function setupImpoundForm(data) {
		...
		$('#weeks').attr("placeholder", `0 - ${rules.maxWeeks} Semanas`);
		$('#days').attr("placeholder", `0 - ${rules.maxDays} Dias`);
		$('#hours').attr("placeholder", `0 - ${rules.maxHours} Horas`);
		$('#fee').attr("placeholder", `${rules.minFee} - ${rules.maxFee}`);
		$('#reason').attr("placeholder", `Entre com uma descrição detalhada de no mínimo ${rules.minReasonLength} caracteres ou mais`)
	}
	...
	function validateImpoundForm () {
		...
		if(String(weeks).length < 1 || parseInt(weeks) < 0 || parseInt(weeks) > rules.maxWeeks || String(days).length < 1 || parseInt(days) < reason.minDays || parseInt(days) > rules.maxDays || String(hours).length < 1 || parseInt(hours) < 0 || parseInt(hours) > rules.maxHours) {
			errors.append(`<small>&#9679; Semanas tem que ser 0 ou menos que ${rules.maxWeeks}, dias tem que ser 0 ou menos que ${rules.maxDays} e horas tem que ser 0 ou menos do que ${rules.maxHours}.</small>`);
			success = false;
		}

		if(fee.isNaN || String(fee).length < 1 || parseInt(fee) < rules.minFee || parseInt(fee) > rules.maxFee) {
			errors.append(`<small>&#9679; Taxa não pode ser menor que ${rules.minFee} ou mais de ${rules.maxFee}</small>`);
			success = false;
		}

		if(reason.length < rules.minReasonLength || reason.length > 10000) {
			errors.append(`<small>&#9679; Razão para a apreensão não pode ser menor que ${rules.minReasonLength} caracteres.</small>`);
			success = false;
		}
		...
		for(var i = 0; i < data.vehicles.length; i++) {
			...
			if(releasedate.getTime() > currentdate.getTime() || data.user.money < data.vehicles[i].fee || data.vehicles[i].hold_m || data.vehicles[i].hold_o) {
				button = `<td>
					...
					<button class="btn pay success" id="pay${i}" disabled>Pagar</button>
				</td></tr>`
			} else {
				button = `<td>
					<button class="btn info mr" id="info${i}">Info</button>
	...
	function setupAdminTerminal (data) {
		...
		for(var i = 0; i < data.vehicles.length; i++) {
			...
			if((data.job.name == "police" && data.vehicles[i].hold_o) || (data.job.name == "mecano" && data.vehicles[i].hold_m)) {
				var button = `<td>
					<button class="btn info mr" id="info${i}">Info</button>
					<button class="btn unlock success" id="${i}">Desbloquear</button>
				</td></tr>`
				...
				var button = `<td>
				<button class="btn info mr" id="info${i}">Info</button>
				<button class="btn unlock success" id="${i}" disabled>Desbloquear</button>
				</td></tr>`
			}
