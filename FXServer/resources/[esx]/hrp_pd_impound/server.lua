	...
	MySQL.Async.execute('INSERT INTO `h_impounded_vehicles` VALUES (@plate, @officer, @mechanic, @releasedate, @fee, @reason, @notes, CONCAT(@vehicle), @identifier, @hold_o, @hold_m)',
		{
			...
			if (rowsChanged == 0) then
				TriggerClientEvent('esx:showNotification', _source, 'Não foi possível apreender o veículo')
			else
				MySQL.Async.execute('UPDATE owned_vehicles SET fourrieremecano = true WHERE plate = @plate',
				{
					['@plate'] 			= form.plate
				})
				TriggerClientEvent('esx:showNotification', _source, 'Veículo apreendido')
...
AddEventHandler('HRP:Impound:UnimpoundVehicle', function (plate)
	...
	if (xPlayer.getMoney() < veh[1].fee) then
		...
		MySQL.Async.execute('DELETE FROM `h_impounded_vehicles` WHERE `plate` = @plate',
		{
			...
			MySQL.Async.execute('UPDATE owned_vehicles SET fourrieremecano = false WHERE plate = @plate',
				{
					['@plate'] 			= plate
				})
	end
