  ...
  Citizen.Wait(0)
    ...
    if car ~= 0 and (wasInCar or IsCar(car)) then
      ...
      if IsControlJustReleased(0, 47) and GetLastInputMethod(0) then
        beltOn = not beltOn 
        if beltOn then 
		  ESX.ShowNotification('Cinto de segurança ~g~COLOCADO~s~')
		  ...
		  isUiOpen = true 
		   ...
		   ESX.ShowNotification('Cinto de segurança ~r~RETIRADO~s~') 
...
RegisterNetEvent('nui:cinto')
AddEventHandler('nui:cinto', function()
Citizen.CreateThread(function()
  while true do
  Citizen.Wait(0)
