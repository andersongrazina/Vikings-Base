...
ESX.RegisterServerCallback('esx_mecanojob:getItemAmount', function(source, cb, item)
    local xPlayer = ESX.GetPlayerFromId(source)
    local qtt = xPlayer.getInventoryItem(item).count
    cb(qtt)
end)

RegisterServerEvent('esx_mecanojob:giveChave')
AddEventHandler('esx_mecanojob:giveChave', function(target, item)
	local _source = source
...
RegisterServerEvent('esx_mecanojob:startRepair')
AddEventHandler('esx_mecanojob:startRepair', function()
			local xPlayer = ESX.GetPlayerFromId(source)
			local FixKitQuantity = xPlayer.getInventoryItem('fixkit').count
			
			xPlayer.removeInventoryItem('fixkit', 1)
			TriggerClientEvent('esx:showNotification', source, ('Você usou 1x ~y~Kit de Reparo'))
...
AddEventHandler('esx_mecanojob:onNPCJobMissionCompleted', function()
	...
	TriggerClientEvent("esx:showNotification", _source, 'Você recebeu R$ '.. total)
end)
...
ESX.RegisterUsableItem('fixkit', function(source)
	...
	if xPlayer.job.name == 'mecano' or xPlayer.job.name == 'cardealer' then
		xPlayer.removeInventoryItem('fixkit', 1)

		TriggerClientEvent('esx_mecanojob:onFixkit', _source)
		TriggerClientEvent('esx:showNotification', _source, _U('you_used_repair_kit'))
	else 
		TriggerClientEvent('esx:showNotification', _source, 'Você precisa ser mecânico para usar esse item.')
	end
end)
