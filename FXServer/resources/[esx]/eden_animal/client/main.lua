...
Citizen.CreateThread(function()
	while true do
		...
		if IsControlJustPressed(0, Keys['F5']) and GetLastInputMethod(2) and not ESX.UI.Menu.IsOpen('default', GetCurrentResourceName(), 'pet_menu') then
			OpenPetMenu()
		end
	end
end)
...
AddEventHandler("K9:ToggleAttack", function(target)
        ...
        TaskPutPedDirectlyIntoMelee(ped, GetPlayerPed(target), 0.0, -1.0, 0.0, 0)
