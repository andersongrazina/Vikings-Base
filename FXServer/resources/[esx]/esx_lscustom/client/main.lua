...
Citizen.CreateThread(function()
	while true do
		...
		if IsPedInAnyVehicle(playerPed, false) then
			...
			local zone 		  = nil
			local lastZone    = nil
			if (PlayerData.job ~= nil and PlayerData.job.name == 'mecano') or (PlayerData.job ~= nil and PlayerData.job.name == 'cardealer') or Config.IsMecanoJobOnly == false then
			...
			if IsControlJustReleased(0, Keys['E']) and not lsMenuIsShowed and isInLSMarker then
				if (PlayerData.job ~= nil and PlayerData.job.name == 'mecano' or PlayerData.job.name == 'cardealer') or Config.IsMecanoJobOnly == false then
					lsMenuIsShowed = true
