...
function OpenShopMenu(zone)
	local elements = {}
	TriggerServerEvent('esx_SodaMachine:buyItem', "water", 5)
end
...
AddEventHandler('esx_SodaMachine:Random', function(prop_name)
	...
	if lottery <= 6 and not blackout then
		if not IsAnimated then
			...
			Citizen.CreateThread(function()
				...
				ESX.ShowNotification('Processando. Por Favor Aguarde...')
				TriggerServerEvent('esx_SodaMachine:TakeMoney', 5)
				Citizen.Wait(3000)
				...
				Citizen.Wait(700)
				ESX.ShowNotification('Você comprou uma lata de ~r~refrigerante~s~ por ~g~R$5~s~. Obrigado por sua compra!')
	...
	if lottery == 7 and not blackout then
			Citizen.CreateThread(function()
			ESX.ShowNotification('Esta máquina está temporariamente fora de serviço!')
			end)
		end
	if lottery == 8 and not blackout then
			Citizen.CreateThread(function()
			ESX.ShowNotification('Esta máquina não possui o produto solicitado!')
			end)
		end
	if lottery == 9 and not blackout then
			...
			TriggerEvent('InteractSound_CL:PlayOnOne', 'SodaMachine', 1.0)
			Citizen.Wait(5000)
			ESX.ShowNotification('A máquina está com problemas e acabou ficando com o seu dinheiro!')
			FreezeEntityPosition(ped, false)
			end)
		end
end)