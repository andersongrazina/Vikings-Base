...
AddEventHandler('esx:setJob', function(job)
	  ...
	  while checking do
		...
		if PlayerData.job ~= nil and (PlayerData.job.name == 'police' or PlayerData.job.name == 'ambulance') then
		  if distance < 100.0 then
			  DrawMarker(22, x, y, z+0.99, 0, 0, 0, 180.0, 0, 0, 0.7, 0.7, 0.5001, 255, 255, 255, 200, 0, 0, 0, 0)
			  if distance < 2.0 then
				 DrawText3D(x,y,z, 'Pressione [~g~E~s~] para investigar o corpo', 0.4)
  ...
  function Notification(x,y,z)
	  ...
	  while (timestamp + 4500) > GetGameTimer() do
		  Citizen.Wait(0)
		  DrawText3D(x, y, z, 'O dano parece ocorrer aqui', 0.4)
		  checking = false
	  end
  end
  ...
  function OpenDeathMenu(player)
	  ...
	  table.insert(elements, {label = 'Tentar identificar a causa da morte!', value = 'deathcause'})
