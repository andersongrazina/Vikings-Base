...
RegisterServerEvent('esx_legacyfuel:devolverDinheiro')
AddEventHandler('esx_legacyfuel:devolverDinheiro', function(plate)
	local xPlayer     = ESX.GetPlayerFromId(source)
	
	xPlayer.addMoney(85)
end)

ESX.RegisterServerCallback('esx_legacyfuel:verificarDinheiro', function (source, cb)
	local xPlayer     = ESX.GetPlayerFromId(source)
	
	if xPlayer.getMoney() >= 85 then
