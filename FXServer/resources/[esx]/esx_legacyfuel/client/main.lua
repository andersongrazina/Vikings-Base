		...
		if not InBlacklistedVehicle then
			...
			if nearPump and IsCloseToLastVehicle then
				...
				elseif IsFueling then
					...
					DrawText3Ds(pumpLoc['x'], pumpLoc['y'], pumpLoc['z'], "Pressione ~g~G ~w~para cancelar o abastecimento do veículo. R$~r~" .. price .. " ~w~+  taxa")
				...
				elseif fuel > 95.0 then
					DrawText3Ds(pumpLoc['x'], pumpLoc['y'], pumpLoc['z'], "O tanque do seu veículo está cheio!")
				elseif cash <= 0 then
					DrawText3Ds(pumpLoc['x'], pumpLoc['y'], pumpLoc['z'], "Você atualmente não tem dinheiro suficiente para comprar combustível")
				else
					DrawText3Ds(pumpLoc['x'], pumpLoc['y'], pumpLoc['z'], "Pressione ~g~G ~w~para abastecer seu veículo. R$~r~3.71/~w~galão + taxa")
			...
			elseif nearPump and Config.EnableJerryCans then
				local jerrycan = GetAmmoInPedWeapon(GetPlayerPed(-1), 883325847)
				
				DrawText3Ds(pumpLoc['x'], pumpLoc['y'], pumpLoc['z'], "Pressione ~g~E ~w~para comprar um galão de gasolina. Valor: ~g~R$85.")
				
				if IsControlJustReleased(0, 38) then
					ESX.TriggerServerCallback('esx_legacyfuel:verificarDinheiro', function(hasEnoughMoney)

						if hasEnoughMoney then
							if jerrycan < 1750 then
								RemoveWeaponFromPed(PlayerPedId(), GetHashKey("WEAPON_PETROLCAN"), true, true)
								SetPedAmmo(GetPlayerPed(-1), 883325847, 0)
								GiveWeaponToPed(PlayerPedId(), GetHashKey("WEAPON_PETROLCAN"),2500, true, true)
							else
								TriggerServerEvent('esx_legacyfuel:devolverDinheiro')
								ESX.ShowNotification('O seu ~y~galão de gasolina~s~ está ~y~cheio~s~!')
							end
						else
							ESX.ShowNotification('Você ~r~não tem dinheiro suficiente~s~ para comprar o ~y~galão de gasolina!')
						end
					end)
				end
			...
			elseif NearVehicleWithJerryCan and not nearPump and Config.EnableJerryCans then
				...
				if IsFuelingWithJerryCan then
					DrawText3Ds(coords.x, coords.y, coords.z + 0.5, "Pressione ~g~G ~w~para cancelar o abastecimento do veículo. Atualmente em: " .. fuel .. "% - Galão: " .. jerrycan)
					...
					DrawText3Ds(coords.x, coords.y, coords.z + 0.5, "Pressione ~g~G ~w~para abastecer o veículo com o galão de gasolina")
...
function DisplayHud()
	if IsPedInAnyVehicle(GetPlayerPed(-1), false) and GetSeatPedIsIn(GetPlayerPed(-1)) == -1 then
		...
		DrawTxt("~g~Tanque:~s~ "..fuel, 0.88, 0.95)
	end
end
	...
	while true do
		...
		if vehicle and engine then
			...
			elseif rpm > 0.8 then
				...
				Citizen.Wait(1500)
			elseif rpm > 0.7 then
				rpmfuelusage = fuel - rpm / 5.5
				Citizen.Wait(2000)
			elseif rpm > 0.6 then
				rpmfuelusage = fuel - rpm / 6.6
				Citizen.Wait(3000)
			elseif rpm > 0.5 then
				rpmfuelusage = fuel - rpm / 7.3
				Citizen.Wait(4000)
			elseif rpm > 0.4 then
				rpmfuelusage = fuel - rpm / 8.2
				Citizen.Wait(5000)
			elseif rpm > 0.3 then
				rpmfuelusage = fuel - rpm / 9.0
				Citizen.Wait(6000)
			elseif rpm > 0.2 then
				rpmfuelusage = fuel - rpm / 10.6
				Citizen.Wait(8000)
			else
				rpmfuelusage = fuel - rpm / 11.2
				Citizen.Wait(15000)
			end
