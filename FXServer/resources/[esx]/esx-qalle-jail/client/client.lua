...
function JailLogin()
	...
	ESX.ShowNotification("A última vez que você foi dormir você foi preso, porque você está agora de volta!")
...
function UnJail()
	...
	ESX.ShowNotification("Você está liberado, fique calmo do lado de fora! Boa Sorte!")
end
...
function InJail()
	...
	Citizen.CreateThread(function()
		...
		while jailTime > 0 do
			...
			ESX.ShowNotification("Faltam ~y~" .. jailTime .. " ~s~anos na prisão!")
		...
		while true do
			...
			for p, v in pairs(Config.Teleports) do
				...
				if DistanceCheck <= 7.5 then
					...
					ESX.Game.Utils.DrawText3D(v, "[E] Para Abrir a Porta", 0.4)
...
function DeliverPackage(packageId)
	...
	while Packaging do
		...
		if not IsEntityAttachedToEntity(packageId, PlayerPedId()) then
			...
			ESX.Game.Utils.DrawText3D(DeliverPosition, "[E] Entregar o Pacote", 0.4)
...
function OpenJailMenu()
	ESX.UI.Menu.Open(
		'default', GetCurrentResourceName(), 'jail_prison_menu',
		{
			...
			align    = 'center',
			elements = {
				{ label = "Prender Jogador Próximo", value = "jail_closest_player" },
			...
			ESX.UI.Menu.Open(
          	...
          	function(data2, menu2)
            	...
            	if jailTime == nil then
              		ESX.ShowNotification("O tempo precisa ser em minutos!")
            	else
              		menu2.close()
              		...
              		if closestPlayer == -1 or closestDistance > 3.0 then
                		ESX.ShowNotification("Nenhum jogador próximo!")
					else
						ESX.UI.Menu.Open(
						...
						function(data3, menu3)
						  	...
						  	if reason == nil then
								ESX.ShowNotification("Você precisa colocar alguma coisa aqui!")
						  	else
								menu3.close()
								...
								if closestPlayer == -1 or closestDistance > 3.0 then
								  	ESX.ShowNotification("Não há jogador próximo!")
								else
								  	TriggerServerEvent("esx-qalle-jail:jailPlayer", GetPlayerServerId(closestPlayer), jailTime, reason)
									TriggerServerEvent('esx_qalle_brottsregister:addPrisao', GetPlayerServerId(closestPlayer), reason)
								end
		...
		elseif action == "unjail_player" then
			...
			ESX.TriggerServerCallback("esx-qalle-jail:retrieveJailedPlayers", function(playerArray)
				...
				if #playerArray == 0 then
					ESX.ShowNotification("Sua prisão está vazia!")
					return
				end

				for i = 1, #playerArray, 1 do
					table.insert(elements, {label = "Prisoneiro: " .. playerArray[i].name .. " | Tempo de Prisão: " .. playerArray[i].jailTime .. " anos", value = playerArray[i].identifier })
				end
