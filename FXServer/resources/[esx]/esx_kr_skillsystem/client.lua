-- THIS SCRIPT WAS MADE BY KRILLE.
local Keys = {
...
ESX.TriggerServerCallback('skill:get', function( data )
    ...
    table.insert(elements, {label = 'Fôlego: Baixa', value = ''})

    elseif tonumber(data[1].gym) < 50 then
    table.insert(elements, {label = 'Fôlego: OK', value = ''})

    elseif tonumber(data[1].gym) < 75 then
    table.insert(elements, {label = 'Fôlego: Boa', value = ''})

    elseif tonumber(data[1].gym) < 100 then
    table.insert(elements, {label = 'Fôlego: Máxima', value = ''})
