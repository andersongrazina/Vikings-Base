...
function OpenStealMenu(target, target_id)
    ...
    ESX.TriggerServerCallback('esx_thief:getOtherPlayerData', function(data)
        ...
        if Config.EnableBlackMoney then
            ...
            table.insert(elements, {
              label          = '[' .. _U('black_money') .. '] R$' .. blackMoney,
              value          = 'black_money',
              type           = 'item_account',
              amount         = blackMoney,
            })
                    ...
                    function(data2, menu2)
                        ...
                        if data2.current.action == 'steal' then
							if IsPlayerDead(target) then 
								local lib, anim = 'amb@medic@standing@kneel@idle_a', 'idle_a'
...
Citizen.CreateThread(function()
    while true do
		...
		if IsControlJustPressed(1, Keys['G']) and IsPedArmed(ped, 7) and not IsPedInAnyVehicle(ped, true) then
            ...
            if target ~= -1 and distance ~= -1 and distance <= 2.0 then
                ...
                IsAbleToSteal(target_id, function(err)
                    if(not err) or IsPlayerDead(target) then
