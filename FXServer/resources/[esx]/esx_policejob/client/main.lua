...
function setUniform(job, playerPed)
	TriggerEvent('skinchanger:getSkin', function(skin)
		if skin.sex == 0 then
			...
			if job == 'bullet_wear' or job == 'bullet_avanc_wear' then
...
function OpenCloakroomMenu()
	...
	if grade == 'recruit' then
		...
		table.insert(elements, {label = 'Uniforme Paisana', value = 'paisana_wear'})
	elseif grade == 'soldado' then
		table.insert(elements, {label = _U('police_wear'), value = 'soldado_wear'})
		table.insert(elements, {label = 'Uniforme Paisana', value = 'paisana_wear'})
	elseif grade == 'officer' then
		table.insert(elements, {label = _U('police_wear'), value = 'officer_wear'})
		table.insert(elements, {label = 'Uniforme Paisana', value = 'paisana_wear'})
		table.insert(elements, {label = 'Uniforme ROCAM', value = 'rocam_wear'})
	elseif grade == 'sergeant' then
		table.insert(elements, {label = _U('police_wear'), value = 'sergeant_wear'})
		table.insert(elements, {label = 'Uniforme ROCAM', value = 'rocam_wear'})
		table.insert(elements, {label = 'Uniforme Águia', value = 'aguia_wear'})
		table.insert(elements, {label = 'Uniforme SWAT', value = 'swat_wear'})
		table.insert(elements, {label = 'Uniforme Paisana', value = 'paisana_wear'})
		table.insert(elements, {label = 'Colete Avançado', value = 'bullet_avanc_wear'})
	...
	elseif grade == 'major' then
		table.insert(elements, {label = _U('police_wear'), value = 'major_wear'})
		table.insert(elements, {label = 'Uniforme Águia', value = 'aguia_wear'})
		table.insert(elements, {label = 'Uniforme ROCAM', value = 'rocam_wear'})
		table.insert(elements, {label = 'Uniforme SWAT', value = 'swat_wear'})
		table.insert(elements, {label = 'Uniforme Paisana', value = 'paisana_wear'})
	...
	elseif grade == 'tencoronel' then
		table.insert(elements, {label = _U('police_wear'), value = 'tencoronel_wear'})
		table.insert(elements, {label = 'Uniforme Águia', value = 'aguia_wear'})
		table.insert(elements, {label = 'Uniforme ROCAM', value = 'rocam_wear'})
		table.insert(elements, {label = 'Uniforme SWAT', value = 'swat_wear'})
		table.insert(elements, {label = 'Uniforme Paisana', value = 'paisana_wear'})
	...
	ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'cloakroom',
	{
		...
		if data.current.value == 'citizen_wear' then
			...
			if Config.MaxInService ~= -1 then
				...
				ESX.TriggerServerCallback('esx_service:isInService', function(isInService)
					if isInService then
						CancelEvent('esx_policejob:startTimer')
		...
		if Config.MaxInService ~= -1 and data.current.value ~= 'citizen_wear' and data.current.value ~= 'bullet_wear' then
				...
				if not isInService then
					...
					ESX.TriggerServerCallback('esx_service:enableService', function(canTakeService, maxInService, inServiceCount)
						if not canTakeService then
							...
							TriggerEvent('esx_policejob:startTimer')
		...
		if
			...
			data.current.value == 'aguia_wear' or
			data.current.value == 'rocam_wear' or
			data.current.value == 'swat_wear' or
			data.current.value == 'paisana_wear' or
			data.current.value == 'tencoronel_wear' or
			data.current.value == 'chef_wear' or
			data.current.value == 'boss_wear' or
			data.current.value == 'bullet_avanc_wear' or
...
RegisterNetEvent('esx_policejob:startTimer')
AddEventHandler('esx_policejob:startTimer', function()
	local segundo = 0
	local minuto = 0
	...
	Citizen.CreateThread(function()
		while playerInService == true do
			Citizen.Wait(0)
			DrawTxt('~y~Em serviço:~s~ ('..minuto..' Minutos e '..segundo..' segundos)', 0.17,0.80)
			if playerInService == false then
				break
			end
		end
			...
			ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'vehicle_spawner',
			{
				...
				if foundSpawnPoint then
					ESX.Game.SpawnVehicle(vehicleProps.model, spawnPoint, spawnPoint.heading, function(vehicle)
						...
						CarPlate = string.format('POLICIAL')
...
function OpenPoliceActionsMenu()
	...
	ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'police_actions',
	{
		...
		elements = {
			...
			{label = 'Histórico Criminal',       value = 'criminalrecords'},
			{label = "Menu de Prisão",          value = 'jail_menu'}
		}
	}, function(data, menu)
		...
		if data.current.value == 'citizen_interaction' then
			...
			if PlayerData.job.grade_name == 'boss' or PlayerData.job.grade_name == 'chef' then
				table.insert(elements, { label = 'Dar Porte de Armas', value = 'license_add_weapon' })
			end
			
			if PlayerData.job.grade_name == 'boss' then
				table.insert(elements, { label = 'Congelar Operações', value = 'congelar_operacoes' })
			end
			...
			'default', GetCurrentResourceName(), 'citizen_interaction',
			{
				...
				if closestPlayer ~= -1 and closestDistance <= 3.0 then
					...
					elseif action == 'license_add_weapon' then
						TriggerServerEvent('esx_license:addLicense', GetPlayerServerId(closestPlayer), 'weapon')
						 ESX.ShowNotification('Você deu uma licença para porte de armas para ' .. GetPlayerName(closestPlayer))
					elseif action == 'congelar_operacoes' then
						TriggerServerEvent('esx_policejob:congelarOperacoes', GetPlayerServerId(closestPlayer))
					end
			...
			'default', GetCurrentResourceName(), 'vehicle_interaction',
			{
				...
				elseif DoesEntityExist(vehicle) then
					...
					elseif action == 'impound' then
						...
						TriggerEvent("esx_apreensao:openApreensaoMenu")
...
RegisterNetEvent('esx_policejob:deletarCarro')
AddEventHandler('esx_policejob:deletarCarro', function(carro)
	if DoesEntityExist(carro) then
		ESX.Game.DeleteVehicle(carro)
	end
end)
...
AddEventHandler('esx_policejob:hasEnteredMarker', function(station, part, partNum)
	...
	elseif part == 'HelicopterSpawner' then
		if PlayerData.job.grade_name == 'intendent' or PlayerData.job.grade_name == 'lieutenant' or PlayerData.job.grade_name == 'major' or PlayerData.job.grade_name == 'tencoronel' or PlayerData.job.grade_name == 'chef' or PlayerData.job.grade_name == 'boss' then
				CurrentAction     = 'spawn_heli'
				CurrentActionMsg  = _U('spawn_heli')
				CurrentActionData = {station = station, partNum = partNum}
		else 
			ESX.ShowNotification('Você não tem permissão para solicitar um helicóptero!')
		end
...
function SpawnarHelicoptero(station, partNum)
		...
		if CurrentAction ~= nil then
			...
			if IsControlJustReleased(0, Keys['E']) and PlayerData.job ~= nil and PlayerData.job.name == 'police' then
				...
				elseif CurrentAction == 'spawn_heli' then
					SpawnarHelicoptero(CurrentActionData.station, CurrentActionData.partNum)
