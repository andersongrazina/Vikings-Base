...
Citizen.CreateThread(function ()
  ...
  ESX.TriggerServerCallback('esx_vehicleshop:getVeiculos', function (vehicles)
    Vehicles = vehicles
  end)
end)
    ...
    function (data, menu)
      ...
      ESX.UI.Menu.Open(
        'default', GetCurrentResourceName(), 'shop_confirm',
        {
          ...
          align = 'top-left',
          elements = {
            {label = '' .. vehicleData.name, value = 'yes'},
            {label = _U('back'), value = 'no'},
          },
        },
        function (data2, menu2)
          if data2.current.value == 'yes' then
             sendNotification(_U('contact_dealer'), 'warning', 5000)
          end
