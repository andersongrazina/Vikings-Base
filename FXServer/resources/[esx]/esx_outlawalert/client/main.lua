...
AddEventHandler('esx_outlawalert:outlawNotify', function(alert)
	if isPlayerWhitelisted then
		ESX.ShowAdvancedNotification('COMPOM Venus CITY', '~r~Ocorrência', (alert), 'CHAR_CALL911', 8)
	end
end)
...
Citizen.CreateThread(function()
	while true do
		...
		elseif IsPedShooting(playerPed) and GetSelectedPedWeapon(playerPed) ~= GetHashKey("WEAPON_PETROLCAN") and GetSelectedPedWeapon(playerPed) ~= GetHashKey("WEAPON_STUNGUN") and Config.GunshotAlert then
