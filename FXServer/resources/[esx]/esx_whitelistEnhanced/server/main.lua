...
AddEventHandler("playerConnecting", function(playerName, reason, deferrals)
	...
	if not found then
		reason('Você não possui Visto(Whitelist) neste servidor. contate-nos no discord.\nDiscord: '..Config.CommunityLink..'\nSteam(Hex): '..steamID)
		deferrals.done('Você não possui Visto(Whitelist) neste servidor. contate-nos no discord.\nDiscord: '..Config.CommunityLink..'\nSteam(Hex): '..steamID)
		CancelEvent()
	...
	if (onlinePlayers >= Config.PlayersToStartRocade or #PriorityList > 0)  and not isVip then
			...
			for i,k in pairs(playersWaiting) do
				...
				if(#PriorityList == 0) then
					...
					if(onlinePlayers < Config.PlayersToStartRocade and k == steamID and i == firstIndex) then
						...
						table.insert(inConnection, _source)
		...
		if Config.EnableAntiSpam then
			...
			for i = 1, Config.WaitingTime, 1 do
				deferrals.update('Proteção anti-spam: por favor aguarde '..(Config.WaitingTime - i)..' segundos, você será conectado automaticamente!')
				Citizen.Wait(1000)
			end
...
AddEventHandler("esx_whitelistExtended:removePlayerToInConnect", function(xPlayer)
	local _source = source
	if _source ~= nil then
		table.remove(inConnection)
	end
...
TriggerEvent('es:addGroupCommand', 'wlrefresh', 'admin', function (source, args, user)
	loadWhiteList()
	TriggerEvent('esx_whitelistExtended:sendMessage', source, 'Whitelist', 'Whitelist reloaded')
