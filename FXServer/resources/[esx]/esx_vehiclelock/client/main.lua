...
function ToggleVehicleLock(emprego)
	local playerPed = PlayerPedId()
	...
	if not DoesEntityExist(vehicle) then
		...
		if isOwnedVehicle then
			...
			if lockStatus == 1 then -- unlocked
				...
				Citizen.Wait(550)
					ESX.ShowNotification('~y~Veículo: '..GetDisplayNameFromVehicleModel(GetEntityModel(vehicle)).. ' ~r~travado')
				Citizen.Wait(650)
				SetVehicleAlarm(vehicle, false)
			elseif lockStatus == 2 then -- locked
				SetVehicleDoorsLocked(vehicle, 1)
				PlayVehicleDoorOpenSound(vehicle, 1)
				ESX.ShowNotification('~y~Veículo: '..GetDisplayNameFromVehicleModel(GetEntityModel(vehicle)).. ' ~g~destravado')
			end
		end
