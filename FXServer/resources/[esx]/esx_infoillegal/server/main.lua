...
AddEventHandler('esx_infoillegal:Weed', function()
	...
	TriggerEvent('es:getPlayerFromId', source, function(user)
		if (tonumber(user.getMoney()) >= Config.PriceWeedF) then
			...
			TriggerClientEvent('esx:showNotification', source, 'Você ~r~não tem dinheiro suficiente~s~ para pagar a informação!')
		end
	end)
end)
...
RegisterServerEvent('esx_infoillegal:LavagemDinheiro')
AddEventHandler('esx_infoillegal:LavagemDinheiro', function()
	local source = source
	TriggerEvent('es:getPlayerFromId', source, function(user)
		if (tonumber(user.getMoney()) >= Config.PriceLavagem) then
			user.removeMoney(Config.PriceLavagem)
			TriggerClientEvent("esx_infoillegal:LavagemLocal", source)				
		else
			TriggerClientEvent('esx:showNotification', source, 'Você ~r~não tem dinheiro suficiente~s~ para pagar a informação!')
		end
	end)
