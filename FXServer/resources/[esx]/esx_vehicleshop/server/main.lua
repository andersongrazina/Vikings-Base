...
MySQL.ready(function()
	...
	local veiculos = MySQL.Sync.fetchAll('SELECT * FROM vehicles_catalogo')
	...
	for k=1, #veiculos, 1 do
		local veiculo = veiculos[k]

		for v=1, #Categories, 1 do
			if Categories[v].name == veiculo.category then
				veiculo.categoryLabel = Categories[v].label
				break
			end
		end

		table.insert(Veiculos, veiculo)
	end
	...
	TriggerClientEvent('esx_vehicleshop:sendVehicles', -1, Veiculos)
end)
...
ESX.RegisterServerCallback('esx_vehicleshop:getVeiculos', function (source, cb)
	cb(Veiculos)
end)
...
RegisterServerEvent('esx_vehicleshop:darContrato')
AddEventHandler('esx_vehicleshop:darContrato', function ()
	local _source = source
	local xPlayer = ESX.GetPlayerFromId(_source)
	local ContractCount = xPlayer.getInventoryItem('contract').count

			if ContractCount >= 5 then
				TriggerClientEvent('esx:showNotification', source, 'Você não pode fazer mais contratos!')
			else
