	...
	ESX.TriggerServerCallback('esx_vehiclelock:requestPlayerCars', function(isOwnedVehicle)
		...
		if isOwnedVehicle then
		  if globalplate ~= nil or globalplate ~= "" or globalplate ~= " " then
				...
				local closecar = GetClosestVehicle(x, y, z, 2.0, 0, 71)
					...
					local class = GetVehicleClass(vehFront)
						  ...
						  if globalplate ~= nil or globalplate ~= "" or globalplate ~= " " then
							...
							OpenCoffreInventoryMenu(GetVehicleNumberPlateText(vehFront),vehCapacity)
								   ...
								   ESX.ShowNotification('Porta malas está ~r~fechado')
							  end
							end
							else
								ESX.ShowNotification('Não há ~r~veículos~w~ nas proximidades')
							end
						lastOpen = true
		...
		ESX.TriggerServerCallback('esx_vehiclelock:requestPlayerSocietyCars', function(isOwnedSocietyVehicle)
	 
		if isOwnedSocietyVehicle then
		  if globalplate ~= nil or globalplate ~= "" or globalplate ~= " " then
				CloseToVehicle = true
				local vehFront = VehicleInFront()
				local x,y,z = table.unpack(GetEntityCoords(GetPlayerPed(-1),true))
...
function openmenuvehiclePolice()
	local playerPed = GetPlayerPed(-1)
	local coords    = GetEntityCoords(playerPed)
	local vehicle   =VehicleInFront()
	globalplate  = GetVehicleNumberPlateText(vehicle)
...
function OpenCoffreInventoryMenu(plate,max)
  ...
  ESX.TriggerServerCallback('esx_truck:getInventoryV', function(inventory)
    ...
    table.insert(elements, {label = 'Dinheiro sujo R$' .. inventory.blackMoney, type = 'item_account', value = 'black_money'})
...
function OpenPlayerInventoryMenu(owner,plate,max,weight)
  ...
  ESX.TriggerServerCallback('esx_truck:getPlayerInventory', function(inventory)
    ...
    local elements = {}
    table.insert(elements, {label = 'voltar', type = 'retour', value = 'retour'})
    table.insert(elements, {label = 'Dinheiro sujo R$' .. inventory.blackMoney, type = 'item_account', value = 'black_money'})
