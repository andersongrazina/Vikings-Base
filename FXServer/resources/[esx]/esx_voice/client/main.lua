...
function drawLevel(r, g, b, a)
	...
	EndTextCommandDisplayText(0.17, 0.95)
end
...
Citizen.CreateThread(function()
	while true do
		...
		if voice.current == 0 then
			voice.level = _U('normal')
			sizew = voice.default*2
		elseif voice.current == 1 then
			voice.level = _U('shout')
			sizew = voice.shout*2
		elseif voice.current == 2 then
			voice.level = _U('whisper')
			sizew = voice.whisper*2
		end
		
		if NetworkIsPlayerTalking(PlayerId()) and not inCinema then
		local pos = GetEntityCoords(GetPlayerPed(-1))
			drawLevel(41, 128, 185, 255)
			DrawMarker(1, pos.x, pos.y, pos.z-0.95, 0, 0, 0, 0, 0, 0, sizew, sizew, 0.2001, 51, 204, 255, 200, 0, 0, 0, 0)
