...
Citizen.CreateThread(function()
    while true do
        Citizen.Wait(0)
        if ( IsControlJustPressed(0,246) and IsPedInAnyVehicle(GetPlayerPed(-1)) and IsPedInAnyPoliceVehicle(GetPlayerPed(-1)) ) then
            ...
            elseif not deployed and not tracking then
                ...
                if IsEntityAVehicle(trackedveh) then
                  deployed = true
                  tracking = false
                  ESX.ShowAdvancedNotification('COPOM', 'Rastreador', 'Rastreador Ligado.', 'CHAR_CALL911', 7)
									sounddeploy = true
                  TriggerServerEvent('esx_pursuit_helper:sendblip', trackedveh)
                end
            elseif not deployed and tracking then
              ESX.ShowAdvancedNotification('COPOM', 'Rastreador', 'Rastreador Desligado.', 'CHAR_CALL911', 7)
...
AddEventHandler("esx_pursuit_helper:trackerset", function(veh)
  ...
  if not tracking then
    tracking = true
		ESX.ShowAdvancedNotification('COPOM', 'Rastreador', 'Rastreador Conectado.', 'CHAR_CALL911', 7)
		soundconnect = true
    if deployed then
      gpstarget = gps
    ...
    while deployed or tracking do
      ...
      if veh ~= nil then
          if not IsEntityAVehicle(veh) then
            ESX.ShowAdvancedNotification('COPOM', 'Rastreador', 'Sinal Perdido.', 'CHAR_CALL911', 7)
							soundconnlost = true
							deployed = false
              tracking = false
          end
...
AddEventHandler("esx_pursuit_helper:trackerremove", function(gps)
    RemoveBlip(gps)
    ESX.ShowAdvancedNotification('COPOM', 'Rastreador', 'O rastreador foi removido.', 'CHAR_CALL911', 7)
		sounddisconnect = true
    tracking = false
end)
