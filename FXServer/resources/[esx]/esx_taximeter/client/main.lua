...
Citizen.CreateThread(function()
  while true do
    ...
    if IsPedSittingInAnyVehicle(ped) and IsDriver() and IsAppropriateVehicle() and hasMeterAppropriateJob() and PlayerData.job.name == 'taxi' then
		ESX.ShowHelpNotification('Dica: Pressione ~INPUT_SPRINT~ + ~INPUT_DETONATE~ para ligar o taxímetro!')
      meterOwner = true

      if IsControlPressed(1, Keys['LEFTSHIFT']) and IsControlPressed(0, Keys['G']) then
...
function calculateFareAmount()
  if (meterAttrs['meterVisible']) and (meterAttrs['rateType'] == 'distance') and not (meterAttrs['rateAmount'] == nil)  then
    ...
    if start then
      ...
      if Config.DistanceMeasurement == 'km' then
