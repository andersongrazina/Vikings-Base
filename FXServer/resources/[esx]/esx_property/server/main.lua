...
RegisterServerEvent('esx_property:darDinheiro')
AddEventHandler('esx_property:darDinheiro', function(name, price)
	local xPlayer = ESX.GetPlayerFromId(source)

	MySQL.Async.fetchAll('SELECT * FROM owned_properties WHERE name = @name AND owner = @owner AND rented = 0', {
		['@name']  = name,
	...
	function(result)
		for i=1, #result, 1 do
			if xPlayer.identifier == result[1].owner then
				...
				TriggerClientEvent('esx:showNotification', xPlayer.source, 'Esta propriedade é alugada então você não receberá nada!')
			end
		end
	end
	)
...
RegisterServerEvent('esx_property:darBandagem')
AddEventHandler('esx_property:darBandagem', function()
	local _source = source
	local xPlayer = ESX.GetPlayerFromId(_source)
	
	if xPlayer.getMoney() >= 250 then
		TriggerClientEvent('esx:showNotification', _source, 'Você solicitou um ~y~kit de primeiros socorros~s~. Aguarde ~y~10 segundos~s~ que entregaremos à você!')
		xPlayer.removeMoney(250)
		Citizen.Wait(10000)
		xPlayer.addInventoryItem('bandage', 3)
	else
		TriggerClientEvent('esx:showNotification', _source, 'Você não tem dinheiro suficiente para solicitar o kit de primeiros socorros!')
	end
end)

RegisterServerEvent('esx_property:darComida')
AddEventHandler('esx_property:darComida', function()
local _source = source
	local xPlayer = ESX.GetPlayerFromId(_source)
	
	if xPlayer.getMoney() >= 65 then
		TriggerClientEvent('esx:showNotification', _source, 'Você pediu combo de ~y~comida~s~. Aguarde ~y~10 segundos~s~ que entregaremos à você!')
		xPlayer.removeMoney(65)
		Citizen.Wait(10000)
		xPlayer.addInventoryItem('burger', 3)
		xPlayer.addInventoryItem('cola', 3)
	else
		TriggerClientEvent('esx:showNotification', _source, 'Você não tem dinheiro suficiente para pedir comida!')
	end
end)
...
ESX.RegisterServerCallback('esx_property:getPropertyInventory', function(source, cb, owner)
	...
	TriggerEvent('esx_addonaccount:getAccount', 'property_dinheiro_vip', xPlayer.identifier, function(account)
		dinheiroVip = account.money
	end)
...
ESX.RegisterServerCallback('esx_property:getPlayerInventory', function(source, cb)
	...
	local dinheiroVip = xPlayer.getAccount('dinheiro_vip').money
...
RegisterServerEvent('esx_property:washMoney')
AddEventHandler('esx_property:washMoney', function(amount)
	local xPlayer 		= ESX.GetPlayerFromId(source)
