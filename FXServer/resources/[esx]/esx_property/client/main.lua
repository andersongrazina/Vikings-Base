...
function CreateBlips()
	for i=1, #Config.Properties, 1 do
		...
		if property.entering ~= nil then
			...
			SetBlipSprite (Blips[property.name], 434)
			SetBlipDisplay(Blips[property.name], 4)
			SetBlipScale  (Blips[property.name], 1.0)
			SetBlipColour  (Blips[property.name], 69)
...
function OpenPropertyMenu(property)
	...
	if PropertyIsOwned(property) then
		...
		if not Config.EnablePlayerManagement then
			table.insert(elements, {label = 'Comprar: Valor R$: '..property.price, value = 'buy'})
		end
	...
	ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'property',
	{
		...
		elseif data.current.value == 'leave' then
			TriggerServerEvent('esx_property:darDinheiro', property.name, property.price)
			Citizen.Wait(1000)
...
function OpenGatewayAvailablePropertiesMenu(property)
	...
	for i=1, #gatewayProperties, 1 do
		if not PropertyIsOwned(gatewayProperties[i]) then
			table.insert(elements, {
				label = gatewayProperties[i].label .. ' R$' .. ESX.Math.GroupDigits(gatewayProperties[i].price),
				value = gatewayProperties[i].name,
...
function OpenRoomMenu(property, owner)
	...
	if property.isSingle then
		table.insert(elements, {label = 'Comprar Bandagem (R$ 250)', value = 'bandage'})
		table.insert(elements, {label = 'Pedir Comida (R$ 65)', value = 'comida'})
	else
	...
	ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'room',
	{
		...
		elseif data.current.value == 'bandage' then
			TriggerServerEvent('esx_property:darBandagem')
		elseif data.current.value == 'comida' then
			TriggerServerEvent('esx_property:darComida')
		elseif data.current.value == 'wash_money' then
				...
				function(data, menu)
					...
					if amount == nil then
						ESX.ShowNotification('~r~Quantia Inválida')
						ESX.UI.Menu.CloseAll()
					else
						menu.close()
						TriggerServerEvent('esx_property:washMoney', amount)
						ESX.UI.Menu.CloseAll()
					end
				end, function(data2, menu2)
					menu2.close()
...
function OpenRoomInventoryMenu(property, owner)
	...
	ESX.TriggerServerCallback('esx_property:getPropertyInventory', function(inventory)
		...
		if inventory.dinheiroVip > 0 then
			table.insert(elements, {
				label = _U('vip_money', ESX.Math.GroupDigits(inventory.dinheiroVip)),
				type = 'item_account',
				value = 'dinheiro_vip'
			})
		end
...
function OpenPlayerInventoryMenu(property, owner)
	...
	ESX.TriggerServerCallback('esx_property:getPlayerInventory', function(inventory)
		...
		if inventory.dinheiroVip > 0 then
			table.insert(elements, {
				label = _U('dinheiro_vip', ESX.Math.GroupDigits(inventory.dinheiroVip)),
				type = 'item_account',
				value = 'dinheiro_vip'
			})
		end
	...
	while true do
		...
		for i=1, #Config.Properties, 1 do
			...
			if(property.entering ~= nil and not property.disabled and GetDistanceBetweenCoords(coords, property.entering.x, property.entering.y, property.entering.z, true) < Config.DrawDistance) then
				DrawMarker(Config.MarkerType, property.entering.x, property.entering.y, property.entering.z + 1, 0.0, 0.0, 0.0, 0, 180.0, 0.0, Config.MarkerSize.x, Config.MarkerSize.y, Config.MarkerSize.z, Config.MarkerColor.r, Config.MarkerColor.g, Config.MarkerColor.b, 100, true, true, 2, true, false, true, false)
			end
			...
			if(property.exit ~= nil and not property.disabled and GetDistanceBetweenCoords(coords, property.exit.x, property.exit.y, property.exit.z, true) < Config.DrawDistance) then
				DrawMarker(Config.MarkerType, property.exit.x, property.exit.y, property.exit.z + 1, 0.0, 0.0, 0.0, 0, 180.0, 0.0, Config.MarkerSize.x, Config.MarkerSize.y, Config.MarkerSize.z, Config.MarkerColor.r, Config.MarkerColor.g, Config.MarkerColor.b, 100, true, true, 2, true, false, true, false)
			end
			...
			if(property.roomMenu ~= nil and HasChest and not property.disabled and GetDistanceBetweenCoords(coords, property.roomMenu.x, property.roomMenu.y, property.roomMenu.z, true) < Config.DrawDistance) then
				DrawMarker(Config.MarkerType, property.roomMenu.x, property.roomMenu.y, property.roomMenu.z + 1, 0.0, 0.0, 0.0, 0, 180.0, 0.0, Config.MarkerSize.x, Config.MarkerSize.y, Config.MarkerSize.z, Config.RoomMenuMarkerColor.r, Config.RoomMenuMarkerColor.g, Config.RoomMenuMarkerColor.b, 100, true, true, 2, true, false, true, false)
			end
		end
...
Citizen.CreateThread(function()
	while true do
		...
		for i=1, #Config.Properties, 1 do
			...
			if(property.entering ~= nil and not property.disabled and GetDistanceBetweenCoords(coords, property.entering.x, property.entering.y, property.entering.z, true) < Config.MarkerSize.x + 1.0) then
				isInMarker      = true
				currentProperty = property.name
				currentPart     = 'entering'
			end

			if(property.exit ~= nil and not property.disabled and GetDistanceBetweenCoords(coords, property.exit.x, property.exit.y, property.exit.z, true) < Config.MarkerSize.x + 1.0) then
				isInMarker      = true
				currentProperty = property.name
				currentPart     = 'exit'
			end

			if(property.inside ~= nil and not property.disabled and GetDistanceBetweenCoords(coords, property.inside.x, property.inside.y, property.inside.z, true) < Config.MarkerSize.x + 1.0) then
				isInMarker      = true
				currentProperty = property.name
				currentPart     = 'inside'
			end

			if(property.outside ~= nil and not property.disabled and GetDistanceBetweenCoords(coords, property.outside.x, property.outside.y, property.outside.z, true) < Config.MarkerSize.x + 1.0) then
				isInMarker      = true
				currentProperty = property.name
				currentPart     = 'outside'
			end

			if(property.roomMenu ~= nil and HasChest and not property.disabled and GetDistanceBetweenCoords(coords, property.roomMenu.x, property.roomMenu.y, property.roomMenu.z, true) < Config.MarkerSize.x + 1.0) then
				isInMarker      = true
				currentProperty = property.name
