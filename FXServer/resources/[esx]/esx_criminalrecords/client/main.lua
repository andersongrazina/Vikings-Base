...
function OpenCriminalRecords(closestPlayer)
    ESX.TriggerServerCallback('esx_qalle_brottsregister:grab', function(crimes)
        ...
        table.insert(elements, {label = 'Adicionar Crimes', value = 'crime'})
        table.insert(elements, {label = '----= Crimes =----', value = nil})
        ...
        ESX.UI.Menu.Open(
            ...
            {
                title    = 'Registro Criminal',
                elements = elements
            },
        function(data2, menu2)

            if data2.current.value == 'crime' and data2.current.value ~= nil then
                ESX.UI.Menu.Open(
                ...
                function(data3, menu3)
                    ...
                    if crime == tonumber(data3.value) then
                        ...
                        ESX.ShowNotification('Registro completo!')
                        Citizen.Wait(100)
                        OpenCriminalRecords(closestPlayer)
                    end
            ...
            function(data3, menu3)
                ...
                if data3.current.value == 'yes' then
                    ...
                    ESX.ShowNotification('Registro Removido!')
                    Citizen.Wait(100)
                    OpenCriminalRecords(closestPlayer)
                else
                    menu3.close()
