...
function OpenAnimationsMenu()
	local elements = {{ label = '-- Parar Animação --', value = 'parar' }}
	...
	ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'animations',
	{
		...
		if data.current.value == 'parar' then
...
Citizen.CreateThread(function()
	while true do
		...
		if IsControlJustReleased(0, Keys['F3']) and GetLastInputMethod(2) and not isDead and not IsPedInAnyVehicle(GetPlayerPed(-1), false) then
			OpenAnimationsMenu()
		end
