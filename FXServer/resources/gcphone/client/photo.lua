...
Citizen.CreateThread(function()
	...
	while true do
		...
		if phone == true then
			ESX.ShowNotification('Pressione a ~b~"Seta para cima" ~s~para alterar pro modo ~y~selfie~s~.')
			ESX.ShowNotification('Pressione ~b~"ESC" ~s~ou ~b~"ESPAÇO" ~s~pora ~r~sair ~s~do modo ~y~selfie~s~.')
			ESX.ShowNotification('Pressione o ~b~"Botão esquerdo" ~s~para ~g~tirar ~s~uma ~y~selfie~s~.')
