		...
		if IsControlJustPressed(1, KeyOpenClose) and GetLastInputMethod( 0 ) then
			ESX.TriggerServerCallback('gcphone:getItemAmount', function(qtty)
				if qtty > 0 then
					...
					TriggerEvent('esx:showNotification', "Você não tem um ~r~celular~s~")
				end
			end, 'phone')
		end
...
AddEventHandler("gcPhone:receiveMessage", function(message, target)
    ...
    if message.owner == 0 then
        local text = '~s~Nova mensagem'
        if ShowNumberNotification == true then
            for _,contact in pairs(contacts) do
                if contact.number == message.transmitter then
                    text = '~s~Nova mensagem de ~y~'.. contact.display
					ESX.ShowAdvancedNotification('~y~SMS', '~s~Notificação de mensagem', text, headshotTxd, 3)
                    break
                end
				if contact.number ~= message.transmitter then
				text = '~s~Nova mensagem de ~y~'.. message.transmitter
...
RegisterNetEvent('gcphone:FecharCelular')
AddEventHandler('gcphone:FecharCelular', function()
		menuIsOpen = false
		SendNUIMessage({show = false})
		PhonePlayOut()
end)
