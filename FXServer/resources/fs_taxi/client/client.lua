...
RegisterCommand('taxi', function()
	TriggerServerEvent('fs_taxi:check')
end)

RegisterNetEvent('fs_taxi:corrida')
AddEventHandler('fs_taxi:corrida', function()
						...
						if IsPedInVehicle(playerPed, taxiVeh, false) then
							...
							if not DoesBlipExist(GetFirstBlipInfoId(8)) then
								...
								if GetGameTimer() > TaxiInfoTimer + 1000 and GetGameTimer() < TaxiInfoTimer + 10000 then
									DisplayHelpMsg('Selecione seu destino no mapa, em seguida, pressione ~INPUT_PICKUP~ para iniciar a corrida.')
								end
