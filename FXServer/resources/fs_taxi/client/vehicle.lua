	...
	if IsModelValid(taxiModel) then
		if IsThisModelACar(taxiModel) then
			...
			if not DoesEntityExist(taxiVeh) then
				...
				DisplayNotify("Chamada de táxi", "O taxista está a caminho, aguarde o taxi neste mesmo local!")
				...
				DisplayNotify("Chamada de táxi", "Todos os nossos motoristas estão ocupados no momento.")
			end
		end
	end	
end
