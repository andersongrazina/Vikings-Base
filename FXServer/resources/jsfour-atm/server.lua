...
ESX.RegisterServerCallback('jsfour-atm:getUser', function(source, cb)
  ...
  function (result)
    if (result[1] ~= nil) then
      MySQL.Async.fetchAll('SELECT account, code FROM jsfour_atm WHERE identifier = @identifier', {['@identifier'] = identifier},
      function (resulto)
...
AddEventHandler('jsfour-atm:insert', function(amount)
	...
	if amount > xPlayer.getMoney() then
		...
		TriggerClientEvent('esx:showAdvancedNotification', _source, 'Banco Vikins', 'Depósito de Dinheiro', 'Você depositou ~y~'..amount..' ~s~R$! Obrigado por sua preferência!', 'CHAR_BANK_MAZE', 9)
	end
end)
...
AddEventHandler('jsfour-atm:take', function(amount)
	...
	if amount > accountMoney then
		...
		TriggerClientEvent('esx:showAdvancedNotification', _source, 'Banco Vikins', 'Retirada de Dinheiro', 'Você retirou ~y~'..amount..' R$~s~! Obrigado por sua preferência!', 'CHAR_BANK_MAZE', 9)
	end
end)
  ...
  function (result)
    if (result[1] ~= nil) then
      ...
      local senPlayer    = ESX.GetPlayerFromId(_source)
        ...
        function (result)
          if (result[1] ~= nil) then
            TriggerClientEvent('esx:showNotification', _source, 'Você enviou ' .. amount .. '~s~ R$ para ' .. result[1].firstname .. ' ' .. result[1].lastname)
            TriggerClientEvent('esx:showNotification', recPlayer.source, 'Você recebeu ' .. amount .. '~s~ R$ enviados para você de ' .. result[1].firstname .. ' ' .. result[1].lastname)
          end
        end)
      end
    end
...
AddEventHandler('jsfour-atm:createAccount', function( src )
  ...
  function (result)
    if (result[1] == nil) then
      ...
      function (result)
        if (result[1] == nil) then
          MySQL.Async.execute('INSERT INTO jsfour_atm (identifier, account, code) VALUES (@identifier, @account, @code)',
            {
