...
ESX.Game.SpawnVehicle = function(modelName, coords, heading, cb)
	...
	Citizen.CreateThread(function()
		...
		local classe  = GetVehicleClassFromName(model)
		local id      = NetworkGetNetworkIdFromEntity(vehicle)
		
		local veh = AddBlipForEntity(vehicle)
		if classe == 8 then
		SetBlipSprite(veh, 226)
		SetBlipScale(veh, 0.8)
		SetBlipColour(veh, 82)
		else
		SetBlipSprite(veh, 225)
		SetBlipColour(veh, 82)
		end
		if classe == 14 then
		SetBlipSprite(veh, 427)
		SetBlipColour(veh, 82)
		end
		BeginTextCommandSetBlipName("STRING")
		AddTextComponentString('Veículo Pessoal')
		EndTextCommandSetBlipName(veh)
